...
interface MarksImportDialogProps {
  classId: string;
  examTypeId: string;
  subjectId: string;
  disabled?: boolean;
...
export function MarksImportDialog({
  ...
  async function runImport(dryRun: boolean) {
    if (!file || !classId || !examTypeId || !subjectId) return;
  ...
  const disabledAll = !classId || !examTypeId || !subjectId || disabled;
    ...
    >
      ...
      <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV with <code>admission_no</code> or <code>roll_number</code> and{" "}
            <code>marks_obtained</code>. Students with blank marks are skipped.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="rounded-lg border border-gray-200 dark:border-border p-3 space-y-2">
              ...
              {summary.committed > 0 && (
                <p className="text-xs text-blue-600 dark:text-blue-400 flex items-start gap-1">
                  ...
                  Marks saved. You can close this dialog.
                </p>
              )}
            </div>
          )}
                ...
                <TableBody>
                  {rows.map((r) => (
                    ...
                    >
                      ...
                      <TableCell className="text-xs">{r.admission_no || "—"}</TableCell>
                      ...
                      <TableCell className="text-xs">
                        {r.marks_obtained === null ? "skip" : r.marks_obtained}
                      </TableCell>
                      <TableCell className="text-xs">
                        {r.ok ? (
