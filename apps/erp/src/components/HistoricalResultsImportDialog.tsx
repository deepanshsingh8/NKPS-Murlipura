...
// Historical results bulk-import dialog. Mirrors HistoricalFeesImportDialog
// but for "Result GreenSheet" XLSX files from the previous ERP software.
...
export function HistoricalResultsImportDialog({
  triggerLabel = "Bulk import historical results",
  onImported,
}: Props) {
  ...
  const [subjectsInFile, setSubjectsInFile] = useState<string[]>([]);
  ...
  async function runImport(dryRun: boolean) {
    ...
    try {
      ...
      const res = await fetch("/api/results/historical-import", {
        method: "POST",
  ...
  const canCommit = useMemo(() => {
    ...
    if ((summary.results_to_create ?? 0) === 0) return false;
    return true;
    ...
    >
      ...
      <DialogContent className="sm:max-w-4xl max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a &quot;Result GreenSheet&quot; XLSX exported from the previous
            ERP software (one file per class, multiple sheets supported). Auto-creates
            any missing classes/sections/streams, the{" "}
            <code> Half Yearly (Imported …) </code> and <code> Annual (Imported …) </code>
            exam types, and any missing subjects.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {subjectsInFile.length > 0 && (
            <div className="rounded-lg border border-gray-200 dark:border-border p-3 text-xs">
              <span className="text-gray-500 dark:text-gray-400">Subjects detected: </span>
              {subjectsInFile.map((s) => (
          ...
          {summary && (
            <div className="rounded-lg border border-gray-200 dark:border-border p-3 space-y-2">
              <div className="flex items-center flex-wrap gap-2 text-sm">
                <Badge variant="outline">Total {summary.total_rows} students</Badge>
                <Badge variant="outline" className="bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400 border-green-200 dark:border-green-800">
                  Will apply {summary.results_to_create} marks rows
                </Badge>
        ...
        <DialogFooter>
          ...
          >
            ...
            Import {summary?.results_to_create ?? 0} marks
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
