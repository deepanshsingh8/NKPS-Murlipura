...
export default function TeacherNonScholasticPage() {
  ...
  // Fetch teacher's assigned classes, parent subjects, exam types, and default
  // non-scholastic scale on mount.
  useEffect(() => {
    async function bootstrap() {
      ...
      if (tid) {
        ...
        setClasses(
          Array.from(uniq.values()).sort((a, b) => a.sort_order - b.sort_order)
        );
      }
  ...
  const fetchStudentsAndGrid = useCallback(async () => {
    ...
    // Load any existing assessments for these sub-subjects in this exam.
  ...
  async function handleSave() {
    ...
    for (const s of students) {
      for (const sub of subSubjects) {
        ...
        // Send both set + cleared cells so the server honours clears via delete.
        payloadEntries.push({
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-2xl font-bold text-navy-900 dark:text-white">
          Non-Scholastic Entry
        </h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Pick a class, exam, and co-scholastic subject to grade students.
        </p>
      </div>
